...
***Introduction***
The presented R script implements a quantitative trading strategy for the second assets group, utilizing mean-reverting techniques on currency (CAD) and precious metal (XAU) pairs. The strategy involves the calculation of short-term EMAs, medium-term SMAs, and long-term EMAs, along with volatility measures. Positions are then determined based on deviations from these indicators. The script evaluates the strategy's performance for different quarters, focusing on net profit, net Sharpe Ratio (SR), and net Calmar Ratio (CR). The analysis aims to identify optimal trading periods and assess the strategy's risk-adjusted returns over time. The provided visualizations and statistics offer insights into the strategy's effectiveness and guide decision-making for potential future refinements.
